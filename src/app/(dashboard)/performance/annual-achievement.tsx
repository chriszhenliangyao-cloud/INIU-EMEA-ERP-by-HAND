...
export function AnnualAchievement({
  ...
  const fmt = (v: number | null) => v == null ? '—' : isVol ? `${fmtNum(Math.round(v))} u` : `€${fmtNum(Math.round(v))}`
        ...
        <div className="inline-flex bg-slate-100 border border-slate-200 rounded-lg p-0.5 ml-1">
          {(['value', 'volume'] as const).map(m => (
            <button key={m} onClick={() => setMetric(m)}
              className={`text-xs font-semibold px-3 py-1 rounded-md capitalize transition-colors ${metric === m ? 'bg-white text-blue-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
